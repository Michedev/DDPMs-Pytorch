...
def x0_to_xt(x_0: torch.Tensor, alpha_hat: torch.Tensor, eps: Optional[torch.Tensor] = None) -> torch.Tensor:
    """
    ...
    :param alpha_hat: the cumulated variance schedule
    :param eps: pure noise from N(0, 1)
    :return: the noised image x_t at step t
    """
    if eps is None:
        eps = torch.randn_like(x_0)
    return torch.sqrt(alpha_hat) * x_0 + torch.sqrt(1 - alpha_hat) * eps
